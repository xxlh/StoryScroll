    ...
    value: function act(obj, props, duration) {
      ...
      for (var prop in props) {
        if (_typeof(props[prop]) == 'object' && obj[prop]) {
          TweenMax.TweenMax.to(obj[prop], duration, _objectSpread2({}, props[prop], {}, commonProps));
          delete props[prop];
        }
      }

      TweenMax.TweenMax.to(obj, duration, props);
    }
  }, {
    key: "action",
      ...
      function triggerActionByPosition(action) {
        ...
        if (this.storyPosition >= action.triggerPosition) {
          if (storedAction.status != 'acting' && storedAction.status != 'done') {
            ...
            var tweenInstance = TweenMax.TweenMax.to(tweenTarget, action.duration, tweenVars);
