...
import { Container, Graphics, TextStyle, Text, Application, Sprite, AnimatedSprite, Texture } from 'pixi.js';
...
function () {
  ...
  _createClass(StoryScroll, [{
    ...
    key: "act",
    value: function act(obj, props, duration) {
      var commonProps = this._getCommonProps(obj, props);

      for (var prop in props) {
        if (_typeof(props[prop]) == 'object' && obj[prop]) {
          TweenMax.to(obj[prop], duration, _objectSpread2({}, props[prop], {}, commonProps));
          delete props[prop];
        }
      }
    ...
    key: "action",
    value: function action(obj, props, duration, triggerPosition) {
      ...
      for (var prop in props) {
        if (_typeof(props[prop]) == 'object' && obj[prop]) {
          var _hash = this._createHash(8);

          obj.actions[_hash] = {
            action: {
              type: 'point',
              propsRoot: prop,
              props: _objectSpread2({}, props[prop], {}, commonProps),
              duration: duration,
              triggerPosition: triggerPosition
            }
          };
          this.actions.push(_objectSpread2({
            sprite: obj,
            hash: _hash
          }, obj.actions[_hash].action));
          delete props[prop];
        }
      }
    ...
    value: function _scrollerCallback(left, top, zoom) {
      ...
      this.scrollDirection == 'y' ? this.containerScroll.y = -this.storyPosition : this.containerScroll.x = -this.storyPosition; // Act

      this.actions.forEach(function (action) {
        if (action.type == 'point') {
          triggerActionByPosition.call(_this2, action);
      ...
      function triggerActionByPosition(action) {
        ...
        if (this.storyPosition >= action.triggerPosition) {
          if (storedAction.status != 'acting' && storedAction.status != 'done') {
            var tweenTarget = action.propsRoot ? action.sprite[action.propsRoot] : action.sprite;

            var tweenVars = _objectSpread2({}, action.props);

            tweenVars.onComplete = function () {
              storedAction.status = 'done';
              action.props.onComplete && action.props.onComplete.call(this);
            };

            tweenVars.onReverseComplete = function () {
              storedAction.status = 'reversed';
              action.props.onReverseComplete && action.props.onReverseComplete.call(this);
            };

            var tweenInstance = TweenMax.to(tweenTarget, action.duration, tweenVars);
    ...
    value: function _defaultSetting(o) {
      ...
      this.backgroundColor = o.bgcolor; // init
    ...
    value: function _windowResize() {
      ...
      setTimeout(function () {
        _this4.scroller.setDimensions(_this4._clientWidth, _this4._clientHeight, scrollerContentWidth, scrollerContentHeight);

        _this4.scroller.scrollTo(scrollerLeft, scrollerTop, false);
      }, 200);
    }
  }, {
    ...
    value: function _createSprite(imgsrc, opt) {
      var newSprite = new Sprite.from(imgsrc);

      this._setProps(newSprite, opt); // this.loaderList.push(imgsrc);
    ...
    value: function _setActions(obj) {
      ...
      obj.act = function (props, duration) {
        return Self.act(obj, props, duration);
      };

      obj.action = function (props, duration, triggerPosition) {
        return Self.action(obj, props, duration, triggerPosition);
      };

      obj.actionByStep = function (props, section, triggerPosition) {
        return Self.actionByStep(obj, props, section, triggerPosition);
      };
    ...
    value: function _setChapterChildren(chapter) {
      ...
      chapter.sprite = function (imgsrc, o) {
        return _this5.sprite(imgsrc, o, chapter);
      };

      chapter.spriteAnimated = function (imgsrcs, o, autoPlay) {
        return _this5.spriteAnimated(imgsrcs, o, autoPlay, chapter);
      };

      chapter.graphic = function (o) {
        return _this5.graphic(o, chapter);
      };

      chapter.text = function (textCont, o, style_o) {
        return _this5.text(textCont, o, style_o, chapter);
      };

      chapter.chapter = function (o) {
        return _this5.chapter(o, chapter);
      };
    }
  }, {
    key: "_createHash",
