...
import { Container, Graphics, TextStyle, Text, Application, Sprite, Texture, AnimatedSprite, Loader } from 'pixi.js';
...
var StoryScroll = /*#__PURE__*/function () {
  ...
  _createClass(StoryScroll, [{
    ...
    key: "spriteVideo",
    value: function spriteVideo(videoUrl, o, _parent) {
      var sprite = this.sprite(videoUrl, o, _parent);
      var videoButton = this.graphic(_objectSpread2({
        o: o
      }, {
        x: o.x + (o.width - 100) / 2,
        y: o.y + (o.height - 100) / 2,
        alpha: 1
      }), _parent);
      sprite.isPlay = false;
      ...
      if (o.hasVideoButton !== false) {
        ...
        videoButton.beginFill(0x0, 0.5).drawRoundedRect(0, 0, 100, 100, 50).endFill().beginFill(0xffffff).moveTo(36, 30).lineTo(36, 70).lineTo(70, 50);
    ...
    value: function spriteAnimated(imgsrcs, o, autoPlay, _parent) {
      ...
      this._ship(sprite, _parent); // console.log(sprite)


      if (autoPlay !== false) {
        sprite.play();
      } else {
        sprite.playByStep = function (props, section, triggerPosition) {
          return _this.playByStep(sprite, props, section, triggerPosition);
        };
      }
      return sprite;
    }
    ...
    value: function text(textCont, o, style_o, _parent) {
      ...
      var text = new Text(textCont, style);
    ...
    key: "playByStep",
    value: function playByStep(obj, fromto, section, triggerPosition) {
      if (triggerPosition === undefined) triggerPosition = this._getSpriteTriggerPosition(obj);
      if (!obj.actions) obj.actions = {};
      ...
      this.sectionAnimations.push(_objectSpread2({
        sprite: obj,
        hash: hash
      }, obj.actions[hash].action));
      return obj;
    ...
    value: function scrollTo(left, top) {
      var scrollerLeft = this.deviceOrientation == 'portrait' ? 0 : left * this._scale || 0;
      var scrollerTop = this.deviceOrientation !== 'portrait' ? 0 : top * this._scale || 0; // console.log(top+"-----"+scrollerTop)

      this.scroller.scrollTo(scrollerLeft, scrollerTop, false);
    }
  }, {
    key: "restLenght",
    value: function restLenght(length) {
    ...
    value: function _scrollerCallback(left, top, zoom) {
      ...
      this.sectionActions.forEach(function (action) {
        triggerActionByStep.call(_this2, action);
      });
      this.pinActions.forEach(function (action) {
        triggerActionSetPin.call(_this2, action);
      });
      this.sectionAnimations.forEach(function (action) {
        triggerAnimatePlayByStep.call(_this2, action);
      });
      ...
      function goonStage() {
        ...
        if (this.nextPool[0][this.scrollDirection] < this.storyPosition + this.viewLength + (this.STAGE_BOUNDARY - 1) / 2 * this.viewLength) {
          ...
          this.stagePoolByLen.sort(function (a, b) {
            return a[_this3.scrollDirection] + a[_this3.scrollDirection ? 'width' : 'height'] - (b[_this3.scrollDirection] + b[_this3.scrollDirection ? 'width' : 'height']);
          });
      ...
      function recallStage() {
        ...
        if (this.prevPool[this.prevPool.length - 1][this.scrollDirection] + _getStageObjWidth(this.prevPool[this.prevPool.length - 1]) > this.storyPosition - (this.STAGE_BOUNDARY - 1) / 2 * this.viewLength) {
          ...
          this.stagePool.sort(function (a, b) {
            return a[_this4.scrollDirection] - b[_this4.scrollDirection];
          });
      ...
      function _markOnStage(obj) {
        ...
        if (obj.children && obj.children.length > 0) obj.children.map(function (child) {
          return _markOnStage(child);
        });
      }
            ...
            if (_typeof(action.props[prop]) == 'object') {
              for (var subprop in action.props[prop]) {
                ...
                if (positionStatus == 'before') {
                  ...
                  if (storedAction.originProps[prop][subprop] !== undefined) action.sprite[prop][subprop] = storedAction.originProps[prop][subprop];
              ...
              if (positionStatus == 'before') {
                if (storedAction.status == 'before') continue;
                if (storedAction.originProps[prop] !== undefined) action.sprite[prop] = storedAction.originProps[prop];
              } else if (positionStatus == 'after') {
                if (storedAction.status == 'after') continue;
      ...
      function triggerAnimatePlayByStep(action) {
        ...
        if (action.triggerPosition <= this.storyPosition && this.storyPosition < action.triggerPosition + action.section) {
          ...
          var midstate = _getMidstate(action.triggerPosition, action.triggerPosition + action.section, this.storyPosition, action.fromto.from, action.fromto.to, action.fromto.speed);

          if (midstate != storedAction.current) _goto(action.sprite, midstate, storedAction);
        } else if (this.storyPosition >= action.triggerPosition + action.section) {
          // 强制达到最终态
          _goto(action.sprite, action.fromto.to, storedAction);
        } else if (this.storyPosition < action.triggerPosition) {
          // 强制回复最终态
          _goto(action.sprite, action.fromto.from, storedAction);
        }

        function _getMidstate(minNum, maxNum, top, from, to, speed) {
          var times = (maxNum - minNum) / speed;
          var delta = Math.floor((top - minNum) / (maxNum - minNum) * (to - from) * times);
          return from + delta % speed;
        }

        function _goto(sprite, frame, storedAction) {
    ...
    value: function _defaultSetting(o) {
      ...
      this.scroller = new Scroller$1(function (left, top, zoom) {
        return _this5._scrollerCallback(left, top, zoom);
      }, {
        zooming: false,
        animating: true,
        bouncing: false,
      ...
      document.addEventListener("touchstart", function (e) {
        _this5.scroller.doTouchStart(e.touches, e.timeStamp);
      ...
      document.addEventListener("touchmove", function (e) {
        ...
        _this5.scroller.doTouchMove(e.touches, e.timeStamp);
      ...
      document.addEventListener("touchend", function (e) {
        ...
        _this5.scroller.doTouchEnd(e.timeStamp);
    ...
    value: function _createContainer(o) {
      ...
      this.load = function () {
        return _this6.app.loader.load.call(_this6.app.loader);
      };

      this.loader.onComplete.add(function (loader) {
        return _this6.loaded = true;
      });
      ...
      this.containerFitWindow = new Container();
    ...
    value: function _windowResize() {
      ...
      setTimeout(function () {
        _this7.scroller.setDimensions(_this7._clientWidth, _this7._clientHeight, scrollerContentWidth, scrollerContentHeight); // Todo: enableX Y scroll


        _this7.scroller.scrollTo(scrollerLeft, scrollerTop, false);
      }, 200);
    }
  }, {
    ...
    value: function _createSprite(imgsrc) {
      var spriteInstance = new Sprite.from(Texture.EMPTY);
      var isVideo = /\.(mp4|mpg|avi|mkv|flv)$/.test(imgsrc);
      spriteInstance.isVideo = false;

      if (!this.useLoader || this.loaded || isVideo) {
        spriteInstance.texture = Texture.from(imgsrc);
    ...
    value: function _createAnimatedSprite(imgsrcs, autoPlay) {
      ...
      var animatedSpriteInstance = new AnimatedSprite([Texture.EMPTY]);
      ...
      if (_typeof(imgsrcs) == 'object' && imgsrcs.length > 0) {
        ...
        if (!this.useLoader || this.loaded) {
          imgsrcs.forEach(function (imgsrc) {
            textures.push(Texture.from(imgsrc));
          });
          animatedSpriteInstance.textures = textures;
        } else {
          imgsrcs.forEach(function (imgsrc) {
            if (!_this8.loader.resources[imgsrc]) _this8.loader.add(imgsrc);
          });
          this.loader.onComplete.add(function (loader) {
            imgsrcs.forEach(function (imgsrc) {
        ...
        var setAnimatedSpriteTextures = function setAnimatedSpriteTextures(animatedSpriteInstance, resource, autoPlay) {
          ...
          for (var imgkey in resource.data.frames) {
            var texture = Texture.from(imgkey);
    ...
    value: function _setChapterChildren(chapter) {
      ...
      chapter.sprite = function (imgsrc, o) {
        return _this9.sprite(imgsrc, o, chapter);
      };

      chapter.spriteVideo = function (videoUrl, o) {
        return _this9.spriteVideo(videoUrl, o, chapter);
      };

      chapter.spriteAnimated = function (imgsrcs, o, autoPlay) {
        return _this9.spriteAnimated(imgsrcs, o, autoPlay, chapter);
      };

      chapter.graphic = function (o) {
        return _this9.graphic(o, chapter);
      };

      chapter.text = function (textCont, o, style_o) {
        return _this9.text(textCont, o, style_o, chapter);
      };

      chapter.chapter = function (o) {
        return _this9.chapter(o, chapter);
      };
    }
  }, {
    key: "_setActions",
    value: function _setActions(obj) {
      ...
      obj.actionByStep = function (props, section, triggerPosition) {
        return _this10.actionByStep(obj, props, section, triggerPosition);
      };

      obj.setPin = function (triggerPosition, section) {
        return _this10.setPin(obj, triggerPosition, section);
      };
    }
  }, {
    key: "_ship",
    value: function _ship(obj, _parent) {
      ...
      if (_parent) {
        ...
        this.nextPool.sort(function (a, b) {
          return a[_this11.scrollDirection] - b[_this11.scrollDirection];
        });
      }
    }
  }, {
