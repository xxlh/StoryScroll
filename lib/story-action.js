...
var TweenMax = _interopDefault(require('gsap/TweenMax'));
...
let action = (() => {
	StoryScroll.prototype.action = function(obj, props, duration, triggerPosition) {
		...
		if (this.useLoader && !this.loaded && triggerPosition===0) {
			this.loader.onComplete.add(loader => {
				setTimeout(() => {
					triggerActionByPosition.call(this, {sprite:obj, hash, ...obj.actions[hash].action});
				}, this.actionDelaysOnLoaded);
			});
		} else {
			this.pointActions.push({sprite:obj, hash, ...obj.actions[hash].action});
		}
